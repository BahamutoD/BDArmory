...
namespace BahaTurret
{
    public class MissileLauncher : PartModule, IBDWeapon
    {
        ...
        [KSPField] public string homingType = "AAM";

        [KSPField] public string targetingType = "none";
        ...
        [KSPField] public string exhaustPrefabPath;

        [KSPField] public string boostExhaustPrefabPath;

        [KSPField] public string boostExhaustTransformName;

        //aero
        [KSPField] public bool aero = false;
        [KSPField] public float liftArea = 0.015f;
        [KSPField] public float steerMult = 0.5f;
        [KSPField] public float torqueRampUp = 30f;
        Vector3 aeroTorque = Vector3.zero;
        float controlAuthority = 0;
        float finalMaxTorque = 0;
        [KSPField] public float aeroSteerDamping = 0;

        [KSPField] public float maxTorque = 90;
        //

        [KSPField] public float thrust = 30;
        [KSPField] public float cruiseThrust = 3;

        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = true, guiName = "Drop Time"),
         UI_FloatRange(minValue = 0f, maxValue = 2f, stepIncrement = 0.1f, scene = UI_Scene.Editor)] public float
            dropTime = 0.4f;

        [KSPField] public float boostTime = 2.2f;
        [KSPField] public float cruiseTime = 45;
        [KSPField] public float cruiseDelay = 0;
        [KSPField] public bool guidanceActive = true;
        [KSPField] public float maxOffBoresight = 45;

        [KSPField] public float maxAoA = 35;

        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = true, guiName = "Decouple Speed"),
         UI_FloatRange(minValue = 0f, maxValue = 10f, stepIncrement = 0.5f, scene = UI_Scene.Editor)] public float
            decoupleSpeed = 0;

        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = false, guiName = "Direction: "),
         UI_Toggle(disabledText = "Lateral", enabledText = "Forward")] public bool decoupleForward = false;


        [KSPField] public float optimumAirspeed = 220;

        [KSPField] public float blastRadius = 150;
        [KSPField] public float blastPower = 25;
        [KSPField] public float blastHeat = -1;
        [KSPField] public float maxTurnRateDPS = 20;
        [KSPField] public bool proxyDetonate = true;

        [KSPField] public string audioClipPath = string.Empty;

        AudioClip thrustAudio;

        [KSPField] public string boostClipPath = string.Empty;

        AudioClip boostAudio;

        [KSPField] public bool isSeismicCharge = false;

        [KSPField] public float rndAngVel = 0;

        [KSPField] public bool isTimed = false;

        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = false, guiName = "Detonation Time"),
         UI_FloatRange(minValue = 2f, maxValue = 30f, stepIncrement = 0.5f, scene = UI_Scene.Editor)] public float
            detonationTime = 2;

        [KSPField(isPersistant = true, guiActive = true, guiActiveEditor = true, guiName = "Cruise Altitude"),
         UI_FloatRange(minValue = 30, maxValue = 2500f, stepIncrement = 5f, scene = UI_Scene.All)] public float
            cruiseAltitude = 500;

        [KSPField] public string rotationTransformName = string.Empty;
        Transform rotationTransform;

        [KSPField] public bool terminalManeuvering = false;

        [KSPField(isPersistant = true, guiActive = true, guiActiveEditor = true, guiName = "In Cargo Bay: "),
         UI_Toggle(disabledText = "False", enabledText = "True", affectSymCounterparts = UI_Scene.All)] public bool
            inCargoBay = false;
        ...
        [KSPField] public bool spoolEngine = false;

        [KSPField] public bool hasRCS = false;
        [KSPField] public float rcsThrust = 1;
        ...
        [KSPField] public float deployedDrag = 0.02f;

        [KSPField] public float deployTime = 0.2f;

        [KSPField] public bool useSimpleDrag = false;
        [KSPField] public float simpleDrag = 0.02f;
        [KSPField] public float simpleStableTorque = 5;

        [KSPField] public Vector3 simpleCoD = new Vector3(0, 0, -1);

        [KSPField] public float agmDescentRatio = 1.45f;
        ...
        [KSPField] public bool decoupleBoosters = false;
        [KSPField] public float boosterDecoupleSpeed = 5;
        [KSPField] public float boosterMass = 0;
        ...
        [KSPField] public string boostTransformName = string.Empty;
        ...
        [KSPField] public float lockedSensorFOV = 2.5f;
        ...
        [KSPField] public float heatThreshold = 200;
        [KSPField] public bool allAspect = false;
        ...
        [KSPField] public float activeRadarRange = 6000;
        public bool activeRadar = false;
        float lastRWRPing = 0;
        [KSPField] public float activeRadarMinThresh = 140;
        [KSPField] public bool radarLOAL = false;
        ...
        //torpedo
        [KSPField] public bool torpedo = false;
        [KSPField] public float waterImpactTolerance = 25;


        //weapon interface
        [KSPField] public string missileType = "missile";
        ...
        //firing paramters
        [KSPField] public float minLaunchSpeed = 0; //only used by AI
        [KSPField] public float minStaticLaunchRange = 10;
        [KSPField] public float maxStaticLaunchRange = 3000;


        //ballistic options
        [KSPField] public bool indirect = false;
        ...
        public override void OnStart(PartModule.StartState state)
        {
            ...
            if (HighLogic.LoadedSceneIsFlight)
            {
                ...
                if (!string.IsNullOrEmpty(exhaustPrefabPath))
                {
                    foreach (var t in part.FindModelTransforms("exhaustTransform"))
                    {
                        GameObject exhaustPrefab =
                            (GameObject) Instantiate(GameDatabase.Instance.GetModel(exhaustPrefabPath));
                ...
                if (!string.IsNullOrEmpty(boostExhaustPrefabPath) && !string.IsNullOrEmpty(boostExhaustTransformName))
                {
                    ...
                    {
                        GameObject exhaustPrefab =
                            (GameObject) Instantiate(GameDatabase.Instance.GetModel(boostExhaustPrefabPath));
                        exhaustPrefab.SetActive(true);
                        foreach (var emitter in exhaustPrefab.GetComponentsInChildren<KSPParticleEmitter>())
                        {
                            emitter.emit = false;
                    ...
                    foreach (var t in part.FindModelTransforms(boostTransformName))
                    {
                        ...
                        foreach (var be in t.GetComponentsInChildren<KSPParticleEmitter>())
                        {
                            if (be.useWorldSpace)
                            {
                                ...
                                {
                                    BDAGaplessParticleEmitter ge =
                                        be.gameObject.AddComponent<BDAGaplessParticleEmitter>();
                                    ge.part = part;
                ...
                foreach (
                    var emitter in part.partTransform.FindChild("model").GetComponentsInChildren<KSPParticleEmitter>())
                {
                    ...
                    {
                        BDAGaplessParticleEmitter gaplessEmitter =
                            emitter.gameObject.AddComponent<BDAGaplessParticleEmitter>();
        ...
        */
        ...
        public override void OnFixedUpdate()
        {
            ...
            if (hasFired && !hasExploded && part != null)
            {
                ...
                //flybyaudio
                float mCamDistanceSqr =
                    (FlightCamera.fetch.mainCamera.transform.position - transform.position).sqrMagnitude;
                ...
                if (!hasPlayedFlyby
                    ...
                    && mCamRelVSqr < 800*800
                    &&
                    Vector3.Angle(vessel.srf_velocity,
                        FlightGlobals.ActiveVessel.transform.position - transform.position) < 60)
                {
            ...
            if (timeIndex > 1f && vessel.srfSpeed > part.crashTolerance)
            {
                ...
                */
                ...
                if (Physics.Linecast(part.transform.position, previousPos, out lineHit, 557057))
                {
                    ...
                    {
                        Debug.Log(part.partInfo.title + " linecast hit on " +
                                  (lineHit.collider.attachedRigidbody
                                      ? lineHit.collider.attachedRigidbody.gameObject.name
                                      : lineHit.collider.gameObject.name));
                        part.temperature = part.maxTemp + 100;
                    }
                }
            }
        ...
        void UpdateGuidance()
        {
            ...
            if (MissileState != MissileStates.Idle && MissileState != MissileStates.Drop) //guidance
            {
                ...
                if (guidanceActive) // && timeIndex - dropTime > 0.5f)
                {
                    ...
                    float turnRateDPS = Mathf.Clamp(((timeIndex - dropTime)/boostTime)*maxTurnRateDPS*25f, 0,
                        maxTurnRateDPS);
                    if (!hasRCS)
                    {
                    ...
                    if (timeIndex > dropTime + boostTime + cruiseTime)
                    {
                        turnRateDPS = atmosMultiplier*
                                      Mathf.Clamp(
                                          maxTurnRateDPS - ((timeIndex - dropTime - boostTime - cruiseTime)*0.45f), 1,
                                          maxTurnRateDPS);
                        if (hasRCS)
                        {
                            turnRateDPS = 0;
                    ...
                    {
                        part.transform.rotation = Quaternion.RotateTowards(part.transform.rotation,
                            Quaternion.LookRotation(targetPosition - part.transform.position, part.transform.up),
                            turnRateDPS*Time.fixedDeltaTime);
                    }
                ...
                else
                {
                    ...
                    if (aero)
                    {
                        aeroTorque = MissileGuidance.DoAeroForces(this, transform.position + (20*vessel.srf_velocity),
                            liftArea, .25f, aeroTorque, maxTorque, maxAoA);
                    }
                }

                if (aero && aeroSteerDamping > 0)
                {
                    part.rb.AddRelativeTorque(-aeroSteerDamping*
                                              part.transform.InverseTransformVector(part.rb.angularVelocity));
                }
        ...
        [KSPField] public float beamCorrectionFactor;
        [KSPField] public float beamCorrectionDamping;
        ...
        void BeamRideGuidance()
        {
            ...
            Ray laserBeam =
                new Ray(
                    targetingPod.cameraParentTransform.position + (targetingPod.vessel.rb_velocity*Time.fixedDeltaTime),
                    targetingPod.targetPointPosition - targetingPod.cameraParentTransform.position);
            Vector3 target = MissileGuidance.GetBeamRideTarget(laserBeam, part.transform.position, vessel.srf_velocity,
                beamCorrectionFactor, beamCorrectionDamping, (timeIndex > 0.25f ? previousBeam : laserBeam));
        ...
        void CruiseGuidance()
        {
            ...
            if (rotationTransform)
            {
                ...
                Vector3 lookUpDirection = Vector3.ProjectOnPlane(cruiseTarget - transform.position, transform.forward)*
                                          100;
                ...
                rotationTransform.localRotation = Quaternion.Lerp(rotationTransform.localRotation,
                    Quaternion.LookRotation(Vector3.forward, lookUpDirection), 0.04f);
                Quaternion finalRotation = rotationTransform.rotation;
                transform.rotation = originalRotation;
                rotationTransform.rotation = finalRotation;

                vesselReferenceTransform.rotation = Quaternion.LookRotation(-rotationTransform.up,
                    rotationTransform.forward);
            }
        ...
        void AAMGuidance()
        {
            ...
            if (targetAcquired)
            {
                ...
                aamTarget = MissileGuidance.GetAirToAirTarget(targetPosition, targetVelocity, targetAcceleration, vessel,
                    out timeToImpact, optimumAirspeed);
                ...
                //proxy detonation
                if (proxyDetonate &&
                    ((targetPosition + (targetVelocity*Time.fixedDeltaTime)) - (transform.position)).sqrMagnitude <
                    Mathf.Pow(blastRadius*0.5f, 2))
                {
                    part.temperature = part.maxTemp + 100;
                }
            }
        ...
        {
            aeroTorque = MissileGuidance.DoAeroForces(this, targetPosition, liftArea, controlAuthority*steerMult,
                aeroTorque, finalMaxTorque, maxAoA);
        }

        void AGMBallisticGuidance()
        {
            Vector3 agmTarget;
            bool validSolution = MissileGuidance.GetBallisticGuidanceTarget(targetPosition, vessel, !indirect,
                out agmTarget);
            if (!validSolution ||
                Vector3.Angle(targetPosition - transform.position, agmTarget - transform.position) >
                Mathf.Clamp(maxOffBoresight, 0, 65))
            {
                Vector3 dToTarget = targetPosition - transform.position;
                Vector3 direction =
                    Quaternion.AngleAxis(Mathf.Clamp(maxOffBoresight*0.9f, 0, 45f),
                        Vector3.Cross(dToTarget, VectorUtils.GetUpDirection(transform.position)))*dToTarget;
        ...
        void UpdateLaserTarget()
        {
            if (targetAcquired)
            {
                if (lockedCamera && lockedCamera.groundStabilized && !lockedCamera.gimbalLimitReached &&
                    lockedCamera.surfaceDetected) //active laser target
                {
                    ...
                    if (guidanceMode == GuidanceModes.BeamRiding && timeIndex > 0.25f &&
                        Vector3.Dot(part.transform.forward, part.transform.position - lockedCamera.transform.position) <
                        0)
                    {
                ...
                else //lost active laser target, home on last known position
                {
                    if (CMSmoke.RaycastSmoke(new Ray(transform.position, lastLaserPoint - transform.position)))
                    {
                        ...
                        float angle = VectorUtils.FullRangePerlinNoise(0.75f*Time.time, 10)*
                                      BDArmorySettings.SMOKE_DEFLECTION_FACTOR;
                        targetPosition = VectorUtils.RotatePointAround(lastLaserPoint, transform.position,
                            VectorUtils.GetUpDirection(transform.position), angle);
                        targetVelocity = Vector3.zero;
                        targetAcceleration = Vector3.zero;
                        lastLaserPoint = targetPosition;
                    }
            ...
            else
            {
                ...
                if (foundCam != null && foundCam.cameraEnabled && foundCam.groundStabilized &&
                    CanSeePosition(foundCam.groundTargetPosition))
                {
        ...
        void ReceiveRadarPing(Vessel v, Vector3 source, RadarWarningReceiver.RWRThreatTypes type, float persistTime)
        {
            ...
            {
                if ((source - VectorUtils.GetWorldSurfacePostion(targetGPSCoords, vessel.mainBody)).sqrMagnitude <
                    Mathf.Pow(50, 2)
        ...
        void UpdateRadarTarget()
        {
            ...
            {
                if (!activeRadar &&
                    ((radarTarget.predictedPosition - transform.position).sqrMagnitude > Mathf.Pow(activeRadarRange, 2) ||
                     angleToTarget > maxOffBoresight*0.75f))
                {
                    if (vrd)
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        RadarUtils.UpdateRadarLock(ray, lockedSensorFOV, activeRadarMinThresh, ref scannedTargets, 0.4f,
                            pingRWR, RadarWarningReceiver.RWRThreatTypes.MissileLock, radarSnapshot);
                        ...
                        else
                        {
                            for (int i = 0; i < scannedTargets.Length; i++)
                            {
                                if (scannedTargets[i].exists &&
                                    (scannedTargets[i].predictedPosition - radarTarget.predictedPosition).sqrMagnitude <
                                    sqrThresh)
                                {
                                    ...
                                    targetPosition = radarTarget.predictedPosition +
                                                     (radarTarget.velocity*Time.fixedDeltaTime);
                                    targetVelocity = radarTarget.velocity;
                                    targetAcceleration = radarTarget.acceleration;
                                    radarFailTimer = 0;
                                    if (!activeRadar && Time.time - timeFired > 1)
                                    {
                                        if (locksCount == 0)
                                        {
                                            RadarWarningReceiver.PingRWR(ray, lockedSensorFOV,
                                                RadarWarningReceiver.RWRThreatTypes.MissileLaunch, 2f);
                                            Debug.Log("Pitbull! Radar missile has gone active.  Radar sig strength: " +
                                                      radarTarget.signalStrength.ToString("0.0"));
                                        }
            ...
            else if (radarLOAL && radarLOALSearching)
            {
                ...
                RadarUtils.UpdateRadarLock(ray, lockedSensorFOV*3, activeRadarMinThresh*2, ref scannedTargets, 0.4f,
                    pingRWR, RadarWarningReceiver.RWRThreatTypes.MissileLock, radarSnapshot);
                ...
                for (int i = 0; i < scannedTargets.Length; i++)
                {
                    ...
                    {
                        float angle = Vector3.Angle(scannedTargets[i].predictedPosition - transform.position,
                            transform.forward);
                ...
                if (lockedTarget.exists)
                {
                    ...
                    if (!activeRadar && Time.time - timeFired > 1)
                    {
                        RadarWarningReceiver.PingRWR(
                            new Ray(transform.position, radarTarget.predictedPosition - transform.position),
                            lockedSensorFOV, RadarWarningReceiver.RWRThreatTypes.MissileLaunch, 2f);
                        Debug.Log("Pitbull! Radar missile has gone active.  Radar sig strength: " +
                                  radarTarget.signalStrength.ToString("0.0"));
                    }
                    return;
        ...
        void UpdateHeatTarget()
        {
            ...
            if (lockFailTimer >= 0)
            {
                Ray lookRay = new Ray(transform.position,
                    heatTarget.position + (heatTarget.velocity*Time.fixedDeltaTime) - transform.position);
        ...
        void CheckMiss()
        {
            ...
            ((targetPosition + (targetVelocity*Time.fixedDeltaTime)) -
             (transform.position + (part.rb.velocity*Time.fixedDeltaTime))).sqrMagnitude;
            if (sqrDist < 160000 ||
                (MissileState == MissileStates.PostThrust &&
                 (guidanceMode == GuidanceModes.AAMLead || guidanceMode == GuidanceModes.AAMPure)))
            {
                checkMiss = true;
            }
            ...
            if (!hasMissed && checkMiss)
            {
                bool noProgress = MissileState == MissileStates.PostThrust &&
                                  (Vector3.Dot(vessel.srf_velocity - targetVelocity,
                                       targetPosition - vessel.transform.position) < 0);
        ...
        public void Detonate()
        {
            ...
            else if (!hasExploded && hasFired)
            {
                ...
                ExplosionFX.CreateExplosion(position, blastRadius, blastPower, blastHeat, sourceVessel,
                    transform.forward, explModelPath, explSoundPath); //TODO: apply separate heat damage
        ...
        void DetonateSeismicCharge()
        {
            if (!hasExploded && hasFired)
            {
                ...
                */
                ...
                SeismicChargeFX.CreateSeismicExplosion(transform.position - (part.rb.velocity.normalized*15),
        ...
        void DoRCS()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                float giveThrust =
                    Mathf.Clamp(
                        Vector3.Project(relV, rcsTransforms[i].transform.forward).magnitude*
                        -Mathf.Sign(Vector3.Dot(rcsTransforms[i].transform.forward, relV)), 0, rcsThrust);
        ...
        void SimpleDrag()
        {
            ...
            (part.rb.GetPointVelocity(part.transform.TransformPoint(simpleCoD)) +
             (Vector3) Krakensbane.GetFrameVelocity()).sqrMagnitude;
            Vector3 currPos = transform.position;
            float drag = deployed ? deployedDrag : simpleDrag;
            float dragMagnitude = (0.008f*part.rb.mass)*drag*0.5f*simSpeedSquared*
                                  (float)
